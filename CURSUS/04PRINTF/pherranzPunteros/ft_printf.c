...
/*   Updated: 2021/04/25 13:54:58 by pherranz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (st->cv == 'c')
		printchar(&st->lenstr, *st, va_arg(st->args, int));
	if (st->cv == 's')
		printstring(&st->lenstr, *st, va_arg(st->args, char *));
	if (st->cv == 'p')
		printpointer(&st->lenstr, st, va_arg(st->args, unsigned long int));
	if (st->cv == 'd' || st->cv == 'i' || st->cv == 'u')
		printint(&st->lenstr, *st, st->args);
	if (st->cv == 'x' || st->cv == 'X')
		printhexu(&st->lenstr, *st, st->args);
	if (st->cv == '%')
		printchar(&st->lenstr, *st, '%');
}
